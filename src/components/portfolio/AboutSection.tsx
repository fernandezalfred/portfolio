...
export default function AboutSection({ data }: AboutSectionProps) {
  ...
  const aboutDataInfo = [
    { label: "Client", value: data?.noofclients || "0" },
  ...
  return (
    ...
    >
      <div className="w-full flex">
        <AnimationWrapper className="rounded-lg w-full grid-flow-row grid grid-cols-1 sm:grid-cols-3 py-9 divide-y-2 sm:divide-y-0 sm:divide-x-2 divide-green-main z-10">
        ...
        <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1">
          <h1 className="leading-[70px] mb-4 text-3xl lg:text-4xl xl:text-5xl font-medium">
            {headingText.split(" ").map((item, index) => (
              <span key={index} className={`${index === 5 ? "text-green-main" : "text-[#000]"}`}>
                {item}{" "}
              </span>
            ))}
          </h1>
      ...
      <div className="grid grid-flow-row sm:grid-flow-col grid-cols-1 sm:grid-cols-2 gap-8">
        <AnimationWrapper className="flex w-full">
          <motion.div variants={setVariants} className="h-full w-full p-4">
            <Image src={about} alt="about image" quality={100} height={414} width={508} loading="eager" />
          </motion.div>
        </AnimationWrapper>
